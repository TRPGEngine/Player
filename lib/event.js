...
let joinSocketRoom = function joinSocketRoom(socket) {
  let app = this;
  ...
  player.user.getGroups(function(err, groups) {
    if(!!err) {
      debug('add room error. ' + err);
      return;
    }
...
exports.login = async function login(data, cb) {
  let app = this.app;
  ...
  try {
    ...
    if(!user) {
      ...
      // 加入到列表中
      if(!!app.player) {
        app.player.list.add(user, socket, db);
        joinSocketRoom.call(app, socket);
      }
...
exports.loginWithToken = async function loginWithToken(data, cb) {
  let app = this.app;
  let socket = this.socket;
  try {
    if(typeof data === 'string') {
...
exports.updateInfo = async function updateInfo(data, cb) {
  let app = this.app;
  let socket = this.socket;
...
exports.changePassword = async function changePassword(data, cb) {
  let app = this.app;
  let socket = this.socket;
  ...
  try {
    ...
    if(!!uuid && !!token) {
      app.storage.connect(function(db) {
        ...
        modelUser.one(cond, function(err, user) {
          if(!!err) {
            throw new Error(err);
          }
...
exports.getSettings = async function getSettings(data, cb, db) {
  ...
  let user = await db.models.player_user.oneAsync({uuid});
  let settings = await user.getSettingsAsync();
...
exports.saveSettings = async function saveSettings(data, cb, db) {
  ...
  if(!settings) {
    ...
    Object.assign(settings.user_settings, userSettings);
    Object.assign(settings.system_settings, systemSettings);
