...
exports.login = async function login(data, cb) {
  ...
  try {
    ...
    let {username, password, platform, device} = data;
    ...
    if(!user) {
      ...
      db.models.player_login_log.create({
        user_uuid: user.uuid,
...
exports.loginWithToken = async function loginWithToken(data, cb) {
  ...
  try {
    ...
    let {uuid, token, platform, device} = data;
    let ip = socket.handshake.address;
    ...
    let db = await app.storage.connectAsync();
    let user = await db.models.player_user.oneAsync({uuid, token});
    ...
    if(!user) {
      ...
      user.last_login = new Date();
      user.last_ip = socket.handshake.address;
      user.save(() => {
        // db.close();
      });
